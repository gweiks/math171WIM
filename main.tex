...
\begin{definition}
Set $C_0 = [0,1]$ and $C_{n+1} = T(C_n)$ for $n \ge 0$. We will refer to this as the Cantor sequence. The Cantor set is
\[
...
\begin{example}
In the example, we can see the first 7 iterations of the Cantor sequence. In the image, the black bars represent a closed interval. Notice that the number of intervals increases and the intervals become shorter as the iteration count increases. Also notice the recursive pattern; if we were to zoom in on $[0,\frac{1}{3}]$, the image would repeat. \\
